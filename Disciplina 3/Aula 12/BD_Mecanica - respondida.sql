...
update funcionario set salario_func = salario_func + 50.00 where (funcao_func = 'Vendedor');

#Reduza em 10% no salário dos funcionários com função de ‘Mecânico’ e sexo ‘Masculino’.
update  funcionario set salario_func = salario_func - (salario_func * 0.1) where (funcao_func like '%_ec_nico')and(sexo_func like '_asculino');
...
desc venda;
update venda set valor_vend = valor_vend * 1.15 where (forma_pag_vend like '%vista') and (valor_vend >=1000); 
...
delete from pagamento where (cod_comp_fk is not null) and (valor_pag < 800) and (data_pag between '2017-01-01' and '2017-06-30');
