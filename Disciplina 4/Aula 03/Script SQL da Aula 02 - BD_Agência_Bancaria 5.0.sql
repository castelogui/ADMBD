...
#			feitas na clausula 'where '

# Selecione o nome e a renda do cliente que possui a maior renda

select
nome_cli as 'Cliente',
renda_cli as 'Maior renda'
from cliente
where
(renda_cli = (select max(renda_cli) from cliente));

# Selecione o nome e a renda do cliente que possui a menor renda
...
# Selecione o nome e a renda do cliente que possui a renda menor do que a soma dos saldos das conta correntes.
select * from Conta_Corrente;

select 
nome_cli as 'Cliente',
renda_cli as 'Renda menor que a soma dos saldos'
from cliente 
where
(renda_cli < (select sum(saldo_cc) from conta_corrente));


#--------------------Subconsultas Avançadas-----------------------
# 		feitas na clausula 'select'

#	Busca as informações da conta corrente na consulta principal (azul). Já na
#	subconsulta (vermelho) busca a soma dos saques realizados na tabela Saque de
#	acordo com cada conta;

select 
conta_corrente.cod_cc as 'ID Conta Corrente',
conta_corrente.numero_cc as 'Numero da Conta Corrente',
(select sum(valor_saq) from saque where(conta_corrente.cod_cc = saque.cod_cc_fk)) as 'Saques'
from conta_corrente;


#	busca dados na tabela Cliente e Conta_Corrente na consulta principal 
# 	utilizando o INNER JOIN e na subconsulta ela verifica os saques realizados pela conta correta;

select 
cliente.nome_cli as 'Cliente',
conta_corrente.numero_cc as 'Numero da CC', 
conta_corrente.saldo_cc as 'Saldo na Conta Corrente', 
(select sum(valor_saq) from saque where(conta_corrente.cod_cc = saque.cod_cc_fk))as Saques
from 
cliente inner join conta_corrente
on (cliente.cod_cli = conta_corrente.cod_cli_fk);

#	busca os me dados da consulta anterior, mas
#	utiliza agora uma segunda subconsulta.
SELECT
cliente.cod_cli AS 'Código do Cliente',
cliente.nome_cli AS 'Nome do Cliente',
conta_corrente.numero_cc AS 'Número da Conta',
conta_corrente.saldo_cc AS 'Saldo em Conta',
(SELECT SUM(valor_saq) FROM saque WHERE (conta_corrente.cod_cc = saque.cod_cc_fk))
as 'Total de Saques',
(SELECT SUM(valor_dep) FROM deposito WHERE (conta_corrente.cod_cc =
deposito.cod_cc_fk)) as 'Total de Depositos'
fROM
cliente LEFT JOIN conta_corrente
oN (cliente.cod_cli = conta_corrente.cod_cli_fk);
...
cliente.nome_cli as 'Nome do Cliente',
conta_corrente.saldo_cc as 'Saldo em Conta',
(select sum(valor_saq) from saque where (conta_corrente.cod_cc = saque.cod_cc_fk)) as 'Total de Saques',
(select sum(valor_dep) from deposito where (conta_corrente.cod_cc = deposito.cod_cc_fk)) as 'Total de Depositos'
FROM
cliente INNER JOIN conta_corrente ON (cliente.cod_cli = conta_corrente.cod_cli_fk)
