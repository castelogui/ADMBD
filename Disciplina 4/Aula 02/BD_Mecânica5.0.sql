...
# Selecione todos os registros de Cidade, porém, substitua a Chave Estrangeira de Estado pelo Nome do Estado.
select * from cidade;
select 
cidade.cod_cid as 'ID Cidade',
cidade.nome_cid as 'Nome Cidade',
estado.nome_est as 'Nome Estado' 
from cidade, estado
where (cidade.cod_est_fk = estado.cod_est) order by nome_cid;
...
from estado, cidade, endereco
where
(cidade.cod_est_fk = estado.cod_est) and (cidade.cod_cid = endereco.cod_cid_fk);

# Faça um consulta que mostre todos os dados de Funcionário, mas substitua as FK por informações compreensíveis;
select 
	funcionario.cod_func as 'ID', 
	funcionario.nome_func as 'Nome', 
	funcionario.cpf_func as 'CPF', 
	funcionario.rg_func as 'RG', 
	funcionario.datanasc_func as 'Data de Nascimento', 
	funcionario.salário_func as 'Salário', 
	funcionario.telefone_func as 'Telefone', 
	funcionario.celular_func as 'Celular', 
	funcionario.função_func as 'Função', 
	funcionario.telefone_func as 'Telefone',
...
from funcionario, sexo, departamento, endereco, cidade
where (funcionario.cod_sex_fk = sexo.cod_sex)
	and (funcionario.cod_dep_fk = departamento.cod_dep)
	and (funcionario.cod_end_fk = endereco.cod_end)
	and (endereco.cod_cid_fk = cidade.cod_cid);

# Faça um consulta que mostre todos os dados de Cliente, mas substitua as FK por informações compreensíveis;
select 
	cliente.cod_cli as 'ID',
	cliente.nome_cli as 'Nome',
	cliente.estadocivil_cli as 'Estado Civil',
	cliente.cpf_cli as 'CPF',
	cliente.rg_cli as 'RG',
	cliente.datanasc_cli as 'Data de Nascimento',
	cliente.rendafamiliar_cli as 'Renda Familiar',
	cliente.telefone_cli as 'Telefone',
    ...
    endereco.numero_end as 'Nº',
    cidade.nome_cid as 'Cidade'    
from cliente, sexo, endereco, cidade
where (cliente.cod_end_fk = endereco.cod_end)
and (cliente.cod_sex_fk = sexo.cod_sex)
and (endereco.cod_cid_fk = cidade.cod_cid);

# Faça um consulta que mostre todos os dados de uma Venda, mas substitua as FK por informações compreensíveis;
select
	venda.cod_vend as 'ID',
	venda.data_vend as 'Data da Venda',
	venda.valortotal_vend as 'Valor Total',
	venda.desconto_vend as 'Desconto',
	venda.formpag_vend as 'Forma de Pagamento',
	venda.quant_parcelas_vend as 'Quantidade de Parcelas',
    funcionario.nome_func as 'Funcionario',
    cliente.nome_cli as 'Cliente'
from venda, funcionario, cliente
where (venda.cod_func_fk = funcionario.cod_func)
and (venda.cod_cli_fk = cliente.cod_cli);

# Faça um consulta que mostre todos os dados de uma Compra, mas substitua as FK por informações compreensíveis;
select
	compra.cod_comp as 'ID',
	compra.data_comp as 'Data da Compra',
	compra.valortotal_comp as 'Valor total',
	compra.formpag_comp as 'Forma de Pagamento',
	funcionario.nome_func as 'Funcionario',
    fornecedor.nomefantasial_forn as 'Fornecerdor - Nome Fantasia',
    fornecedor.razãosocial_forn as 'Fornecedor - Razão Social',
    fornecedor.representante_forn as 'Fornecedor - Representante'
from compra, funcionario, fornecedor
where (compra.cod_forn_fk = fornecedor.cod_forn)
and (compra.cod_func_fk = funcionario.cod_func);


# -----------------------Exercicios da Aula 02-------------------------

# 		Refaça as consultas do Exercício de Fixação I. Mas agora utilize a comparação com JOIN;
...
on (cidade.cod_est_fk = estado.cod_est);
...
endereco.bairro_end as 'Bairro'
from estado 
inner join cidade on (cidade.cod_est_fk = estado.cod_est)
inner join endereco on (cidade.cod_cid = endereco.cod_cid_fk);
...
select 
	...
	endereco.bairro_end as 'Bairro',
	cidade.nome_cid as 'Cidade'
from funcionario inner join sexo on (funcionario.cod_sex_fk = sexo.cod_sex)
...
select
    ...
    cliente.nome_cli as 'Cliente'
from venda
inner join funcionario on(venda.cod_func_fk = funcionario.cod_func)
...
inner join funcionario on (compra.cod_func_fk = funcionario.cod_func);
...
#		Faça uma consulta que mostre o nome do cliente e os nome
#		dos produtos comprados por ele entre 2018 e 2020 (escolha
#		entre usar o JOIN e a Comparação Direta);
...
inner join cliente on (cliente.cod_cli = venda.cod_cli_fk) 
inner join produto on (produto.cod_prod = itens_venda.cod_prod_fk)
where (venda.data_vend between '2018-01-01' and '2020-12-31');
