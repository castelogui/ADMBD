...
insert into Compra values (null, '2020-10-03', 1024.49, 'Vista', 2, 4);
...
#	Selecione o código, nome, cpf, nome do Sexo, dados do Endereço, 
#	nome da Cidade e nome do Estado dos Clientes.

select 
cliente.cod_cli as 'ID Cliente',
...
endereco.bairro_end as 'Rua',
cidade.nome_cid as 'Cidade',
estado.nome_est as 'Estado'
from cliente 
inner join sexo on (cliente.cod_sex_fk = sexo.cod_sex)
inner join endereco on (cliente.cod_end_fk = endereco.cod_end)
inner join cidade on (endereco.cod_cid_fk = cidade.cod_cid)
inner join estado on (cidade.cod_est_fk = estado.cod_est);


#	Selecione o código, nome, cpf, função, nome do Sexo, dados do Endereço, 
#	nome da Cidade, nome do Estado e nome do Departamento dos Funcionários.
select 
funcionario.cod_func as 'ID Funcionario',
funcionario.nome_func as 'Nome Funcionario',
funcionario.cpf_func as 'CPF Funcionario',
funcionario.função_func as 'Função do Funcionario',
sexo.nome_sex as 'Sexo do Funcionario',
endereco.rua_end as 'Rua',
endereco.numero_end as 'Numero',
...
from funcionario
inner join sexo on (funcionario.cod_sex_fk = sexo.cod_sex)
inner join endereco on (funcionario.cod_end_fk = endereco.cod_end)
inner join cidade on (endereco.cod_cid_fk = cidade.cod_cid)
inner join estado on (cidade.cod_est_fk = estado.cod_est)
inner join departamento on (funcionario.cod_dep_fk = departamento.cod_dep);

#	Selecione o nome do Cliente e os dados dos seus Carros.
select * from carro;
select 
cliente.nome_cli as 'Nome do Cliente',
carro.modelo_car as 'Modelo do Carro',
carro.cor_car as 'Cor do Carro',
carro.portas_car as 'Portas do Carro',
carro.placa_car as 'Placa do Carro',
carro.marcar_car as 'Marca do Carro'
from carro left join cliente on (carro.cod_cli_fk = cliente.cod_cli);


#	Selecione código e data da Compra, nome do Fornecedor e os nomes, 
#	quantidade e valor dos Produtos comprados.
...
select 
compra.cod_comp as 'Código da Compra',
compra.data_comp as 'Data da Compra',
fornecedor.nomefantasial_forn as 'Fornecedor',
produto.descrição_prod as 'Produto',
itens_compra.quant_itenc as 'Quantidade Comprada',
itens_compra.valor_itenc as 'Valor do Produto Comprado'
from compra 
inner join fornecedor on (compra.cod_forn_fk = fornecedor.cod_forn)
inner join itens_compra on (itens_compra.cod_comp_fk = compra.cod_comp)
inner join produto on (itens_compra.cod_prod_fk = produto.cod_prod);

#	Selecione o código e data da Venda, nome do Cliente e os nomes, 
#	quantidade e valor dos Produtos vendidos.
select * from venda;
select * from cliente;
...
select 
venda.cod_vend as 'Código da Venda',
venda.data_vend as 'Data da Venda',
cliente.nome_cli as 'Cliente',
produto.descrição_prod as 'Produto',
itens_venda.quant_itenv as 'Quantidade de itens',
itens_venda.valor_itenv as 'Valor do Item'
from venda
inner join cliente on (venda.cod_cli_fk = cliente.cod_cli)
inner join itens_venda on (itens_venda.cod_vend_fk = venda.cod_vend)
inner join produto on (produto.cod_prod = itens_venda.cod_prod_fk);

#	Selecione o código, a data, o valor e a forma de pagamento dos Pagamentos, 
#	o código e data do Caixa, o nome do Funcionário e a descrição, data de vencimento e o valor da Despesas.
...
select 
pagamentos.cod_pag as 'Código Pagamento',
pagamentos.data_pag as 'Data Pagamento',
pagamentos.valor_pag as 'Valor Pagamento',
pagamentos.formapagamento_pag as 'Forma Pagamento',
caixa.dataabertura_cai as 'Data Abertura Caixa',
caixa.datafechamento_cai as 'Data Fechamento Caixa',
funcionario.nome_func as 'Nome Funcionario',
despesas.cod_desp as 'Código Despesa',
despesas.descrição_desp as 'Descrição Despesa',
despesas.datavencimento_desp as 'Data Vencimento Despesa',
despesas.valor_desp as 'Valor Despesa'
from pagamentos
inner join caixa on(pagamentos.cod_cai_fk = caixa.cod_cai)
left join despesas on(pagamentos.cod_desp_fk = despesas.cod_desp)
inner join funcionario on(pagamentos.cod_func_fk = funcionario.cod_func);

#	Selecione o código e o nome do Fornecedor, assim como a quantidade e a soma 
#	do valor total das Compras que ele participou. Dica: Use subconsulta com funções.
...
select
fornecedor.cod_forn as 'ID do Fornecedor',
fornecedor.nomefantasial_forn as 'Nome Fantasia',
(select count(cod_comp) from compra where (compra.cod_forn_fk = fornecedor.cod_forn)) as 'Quantidade de Compras',
(select sum(valortotal_comp) from compra where(compra.cod_forn_fk = fornecedor.cod_forn)) as 'Valor das Compras'
from fornecedor;


#	Secione o código e nome do Cliente, assim como o valor médio e a soma do valor 
#	total das Vendas que ele participou. Dica: Use subconsulta com funções.

select 
cliente.cod_cli as 'ID Cliente',
cliente.nome_cli as 'Nome Cliente',
(select avg(valortotal_vend) from venda where(venda.cod_cli_fk = cliente.cod_cli)) as 'Média de Vendas',
(select sum(valortotal_vend) from venda where(venda.cod_cli_fk = cliente.cod_cli)) as 'Valor total de Vendas'
from cliente;


#	Selecione o código, a data e a forma de pagamento da Venda, o nome do Cliente,
#	assim como a quantidade e a soma do valor total dos Itens Vendidos em cada Venda.  
...
select * from itens_venda;
select 
venda.cod_vend as 'ID',
venda.data_vend as 'Data',
venda.formpag_vend as 'Forma de Pagamento',
cliente.nome_cli as 'Nome Cliente',
(select sum(quant_itenv) from itens_venda 
	where(itens_venda.cod_vend_fk = venda.cod_vend)) as 'Quantidade de Itens',
(select sum(quant_itenv * valor_itenv) from itens_venda 
	where(itens_venda.cod_vend_fk = venda.cod_vend)) as 'Valor tatal de Itens'
from venda
inner join cliente on (venda.cod_cli_fk = cliente.cod_cli);


#	Selecione o nome do Funcionário, assim como a quantidade e a soma do valor 
#	total das Vendas realizadas por cada vendedor. Dica: Use subconsulta com funções.

select * from funcionario;
select * from venda;

select 
funcionario.nome_func as 'Vendedor',
(select count(cod_vend) from venda where (funcionario.cod_func = venda.cod_func_fk)) as 'Quantidade de Vendas',
(select sum(valortotal_vend) from venda where (funcionario.cod_func = venda.cod_func_fk)) as 'Total de Vendas'
from funcionario where (funcionario.função_func like 'Vende%');
