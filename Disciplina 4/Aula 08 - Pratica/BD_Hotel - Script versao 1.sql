...
delimiter $$ 
create procedure classificacaoDoCliente(codigo int)
begin
	declare somaHospedagem float;
    declare nomeCliente varchar(150);
	select (valorDiárias_hosp * (dataCheckout_hosp - dataCheckin_hosp)) 
		into somaHospedagem from hospedagem 
		where(codigo = cod_cli);
    select nome_cli into nomeCliente from cliente where(codigo = cliente.cod_cli);
	if (somaHospedagem >= 1000.00) then
		select concat('O cliente ',nomeCliente,' é VIP') as Classificação;
	else
		if (somaHospedagem between 500.00 and 1000.00) then
			select concat('O cliente ',nomeCliente,' é NORMAL') as Classificação;
		else
			if (somaHospedagem <= 500.00) then
				select concat('O cliente ',nomeCliente,' é POPULAR') as Classificação;
			else
				select Concat('Cliente ', nomeCliente,' sem classificação!') as Classificação;
			end if;
		end if;
	end if;
end $$ delimiter ;
...
-	Retorno mensagens de retorno cada a atualização aconteça ou não
...
delimiter $$
create procedure ajusteValorProduto(percentual float, tipo varchar(10))
begin
	if (tipo = 'mais') then
		if(percentual <= 50) then
            ...
            select 
				cod_prod as Codigo,
                descrição_prod as Descrição,
                valor_prod as 'Valor ajustado a mais' 
			from produto;
		else
			select 'Percentual deve ser menor que 50%' as Message;
		end if;
	elseif (tipo = 'menos') then
		if(percentual <= 20) then
			update produto set valor_prod = valor_prod -(valor_prod *(percentual/100));
            select 
				cod_prod as Codigo,
                descrição_prod as Descrição,
                valor_prod as 'Valor ajustado a menos'
			from produto;
		else
			select 'Percentual deve ser menor que 20%' as Message;
		end if;
	end if;
end $$ delimiter ;

call ajustevalorproduto(50, 'mais');
call ajustevalorproduto(20, 'menos');
